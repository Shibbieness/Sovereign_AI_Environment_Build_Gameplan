"""
lattice_common.py — Shared parsing and helpers for WEAVE and BLOOM validators.

Pure stdlib. No external dependencies. Minimal frontmatter parser handles the
constrained YAML subset that LATTICE uses.
...
def parse_frontmatter(text: str) -> tuple[dict, str]:
    ...
    nested keys via 2-space indent, and the constrained subset LATTICE uses.
...
def find_profile(start_path: Path) -> Path | None:
    """Walk up from start_path looking for lattice.profile.yaml."""
    p = start_path.resolve()
    if p.is_file():
        p = p.parent
    while True:
        candidate = p / "lattice.profile.yaml"
...
# ----------------------------------------------------------------------------
# Heuristic checks usable by either validator
# ----------------------------------------------------------------------------
