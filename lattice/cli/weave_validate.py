#!/usr/bin/env python3
"""
weave_validate.py — Validate that a markdown document (or directory) is
WEAVE-compliant per LATTICE Stage 10 contract.

Usage:
  python weave_validate.py PATH [--json] [--verbose] [--strict]
...
REQUIRED_FRONTMATTER_FIELDS = ["lattice_version", "doc_id", "title"]
...
def validate_doc(path: Path, profile: dict | None, strict: bool = False) -> ValidationReport:
    """Run all 10 WEAVE checks on a single doc."""
    ...
    # Stage 10 — Document-level checks (CASL floor — heuristic)
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="Validate WEAVE compliance for a doc or directory")
